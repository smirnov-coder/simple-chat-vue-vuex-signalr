<template>
    <v-btn light
           color="primary"
           @click.stop="signIn">
        <v-icon left light>mdi-linkedin</v-icon>
        Вход через Linked In
    </v-btn>
</template>
        ...
        methods: {
            signIn() {
                let
                    ...
                    { clientId, redirectUri, scope, state, authorizeEndpoint } = window.globals.LinkedIn;
                ...
                window.open(`${authorizeEndpoint}?${queryString.toString()}`, "linkedin_popup", features);
            },
        },
    };
</script>
